<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Acces $acces
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Acces'), ['action' => 'edit', $acces->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Acces'), ['action' => 'delete', $acces->id], ['confirm' => __('Are you sure you want to delete # {0}?', $acces->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Access'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Acces'), ['action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="access view large-9 medium-8 columns content">
    <h3><?= h($acces->name) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Name') ?></th>
            <td><?= h($acces->name) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($acces->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($acces->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($acces->modified) ?></td>
        </tr>
    </table>
</div>
